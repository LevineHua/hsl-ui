 ...
 * @LastEditors: 华松林
 * @LastEditTime: 2021-12-18 21:42:42
 * @FilePath: /hsl-ui/build/gen-type.js
 */
...
fs.copyFileSync(
  ...
  path.resolve(__dirname, '../lib/hsl-ui.d.ts'),
)
// index.d.ts
const newIndexPath = path.resolve(__dirname, '../lib/index.d.ts')
fs.copyFileSync(path.resolve(__dirname, '../lib/hsl-ui/index.d.ts'), newIndexPath)
const index = fs.readFileSync(newIndexPath)
const newIndex = index.toString().replace(/@hsl-ui\//g, './el-').replace('el-utils', 'utils')
fs.writeFileSync(newIndexPath, newIndex)

// remove ep
fs.rmdirSync(path.resolve(__dirname, '../lib/hsl-ui'), { recursive: true })
...
fs.readdirSync(libDirPath).forEach(comp => {
  if (!noElPrefixFile.test(comp)) {
    if (fs.lstatSync(path.resolve(libDirPath, comp)).isDirectory()) {
      ...
      if(outsideImport.test(imp) || imp.includes('@hsl-ui/')) {
        const newImp = imp.replace(outsideImport, (i, c) => {
          return i.replace(`../${c}`, `../el-${c}`)
        }).replace('@hsl-ui/', '../el-').replace('el-utils', 'utils')
